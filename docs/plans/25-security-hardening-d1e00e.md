...
The platform's defense-in-depth posture: explicit threat model, isolation tiers, hardened container runtimes (gVisor / Kata), supply-chain security, secrets, and incident response. Built on the principle that the _platform_ should be more trustworthy than the apps it runs.
